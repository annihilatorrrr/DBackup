...
- **runner/steps**: Added 4 new unit test files and extended the existing `02-dump.test.ts` to significantly increase coverage for all 5 pipeline step files. `01-initialize.ts` now has 100% statement, branch, function, and line coverage (10 tests covering job-not-found, missing-source, no-destinations, execution creation/skip, source adapter not found, destination adapter skip with warning, all destinations missing, invalid retention JSON fallback). `03-upload.ts` reached 100% statement and line coverage (24 tests: context-not-ready guards, PROCESSING vs. UPLOADING stage selection, compression/encryption pipeline, pipeline failure error wrapping, encryption key failure, checksum and metadata sidecar writing, upload success/failure/false-return paths, Partial status, post-upload integrity verification, verify failure warning, progress and log callbacks, percentage-based detail string). `04-completion.ts` reached 100% statement and line coverage (14 tests: cleanup file-exists/not-exists/no-tempFile, finalize no-execution guard, execution record update, shouldNotify conditions for ALWAYS/SUCCESS_ONLY/FAILURE_ONLY/Partial, notification send success/failure recording, adapter-not-found skip, error log in failure payload, Slack fields rendering, cache refresh failure silent handling). `05-retention.ts` reached 100% statement and line coverage (12 tests: context guards, upload-failed skip, NONE policy skip, no-list-support skip, delete old backups, delete error handling, retention process error, locked-file detection via meta.json read, read error ignored, cache refresh trigger, cache refresh failure silent). Additional tests added to `02-dump.test.ts` cover the `--all-databases` options branch, getDatabases failure fallback, engine edition detection, invalid-JSON databases fallback, post-dump DB discovery, TAR rename skip when already `.tar`, and TAR check catch block.

- **logging**: Achieved 100% statement, branch, function, and line coverage on `src/lib/logging/errors.ts` and `src/lib/logging/logger.ts`. Added `errors.test.ts` (55 tests covering every error class - `DBackupError`, `AdapterError`, `ConnectionError`, `ConfigurationError`, `ServiceError`, `NotFoundError`, `ConflictError`, `ValidationError`, `PermissionError`, `AuthenticationError`, `BackupError`, `RestoreError`, `EncryptionError`, `QueueError`, `ApiKeyError` - plus utility functions `isDBackupError`, `wrapError`, `getErrorMessage`, `getErrorCode`, `withContext` including all edge-case branches). Added `logger.test.ts` (17 tests covering level filtering with `LOG_LEVEL` env var, production JSON format vs. development human-readable format, `console.warn`/`console.error` routing, error code inclusion, stack trace output, child logger context merging, and production fallback to `info` when `LOG_LEVEL` is unset).
- **notifications**: Achieved 100% statement, branch, function, and line coverage on `src/lib/notifications/templates.ts`, `src/lib/notifications/events.ts`, `src/lib/notifications/types.ts`, and `src/lib/notifications/index.ts`. Extended the existing `templates.test.ts` with tests for the previously untested `UPDATE_AVAILABLE`, `CONNECTION_OFFLINE`, and `CONNECTION_ONLINE` templates (including Source vs. Destination label switching, optional `downtime`/`lastError`/`releaseUrl` fields), added comprehensive `RESTORE_COMPLETE` and `RESTORE_FAILURE` tests covering all optional fields (`databaseType`, `storageName`, `backupFile`, `size`) and their false branches, and added a barrel-export block that imports from `@/lib/notifications` to drive `index.ts` coverage.

- **crypto**: Achieved 100% statement, branch, function, and line coverage on `src/lib/crypto/index.ts` and `src/lib/crypto/stream.ts`. Added tests for `stripSecrets` (null/primitive passthrough, sensitive-key replacement, recursive nested objects, arrays, immutability), `encryptConfig`/`decryptConfig` (null/primitive passthrough, flat and deeply nested round-trips, array configs, immutability), `encrypt` error-wrapping (non-EncryptionError wrapped as EncryptionError via invalid hex key that decodes to empty Buffer), `decrypt` EncryptionError re-throw (missing key while input passes format guard), and key-length validation for both `createEncryptionStream` and `createDecryptionStream` (short and long keys). Removed dead unreachable `parts.length !== 3` guard inside `decrypt`'s try block (the outer format guard already guarantees this). Total: 29 tests in `crypto.test.ts`, 6 tests in `crypto-stream.test.ts`.
- **core**: Added 4 unit test files for `src/lib/core/` achieving 100% statement, branch, function, and line coverage on all targeted files - `logs.test.ts` (8 tests: `stageProgress` unknown stage, min/max boundary, midpoint interpolation, negative and over-100 clamping, Completed and Queued edge cases), `registry.test.ts` (8 tests: register and retrieve by id, unknown id returns undefined, overwrite warning, `getAll`, `getDatabaseAdapters`, `getStorageAdapters`, `getNotificationAdapters`, defensive fallback for unknown type), `credential-requirements.test.ts` (11 tests: primary + SSH for MySQL/Postgres, SSH-only for SQLite, SSH_KEY primary for SFTP, ACCESS_KEY for S3 family, SMTP for email, TOKEN for Telegram, undefined for unknown adapter, all DB and S3 adapters present), `retention.test.ts` (2 tests: `DEFAULT_RETENTION_CONFIG` mode is NONE, no simple or smart policy by default).

- **sqlite**: Added 3 unit test files for the SQLite adapter achieving ~97% statement and line coverage - `connection.test.ts` (18 tests: local mode binary check success/failure, default binary fallback, file access failure, SSH mode with missing config/successful connection/missing remote file/connection error, invalid mode, `getDatabases` path parsing, `getDatabasesWithStats` local and SSH with size/table-count success, stat failure, table-count failure, SSH config missing, SSH connection failure), `dump.test.ts` (13 tests: local spawn success/default-binary/non-zero-exit/spawn-error/stderr-logging/timestamps, invalid mode, SSH missing config/success/non-zero-exit/execStream-error/SIGKILL-signal), `restore.test.ts` (16 tests: `prepareRestore` no-op, local success/safety-backup/progress/non-zero-exit/spawn-error/timestamps, invalid mode, SSH missing config/success/progress/size-mismatch/non-zero-exit/SIGTERM-signal/cleanup-in-finally). Total: 47 tests passing.
- **redis**: Added 4 unit test files for the Redis adapter achieving 100% statement, branch, function, and line coverage on all source files - `analyze.test.ts` (trivially 100%), `connection.test.ts` (29 tests: `buildConnectionArgs` pure-function suite, local/SSH `test()` and `getDatabases()` including PONG failure, no-version-in-INFO, `Unknown error` fallback, empty-count-line fallback, and non-Error rejections), `dump.test.ts` (21 tests: local spawn success/failure/empty-file/non-Error, password masking, empty stdout trim, SSH RDB flow with onLog, no-password placeholder, stream exit with signal and null code, stream/writeStream error handlers, and cleanup-exec rejection), `restore.test.ts` (12 tests: `prepareRestore` with default user, non-default user ACL branches, ACL-unavailable catch, PING failure, non-Error rejection; `restore` success, fallback dataDir/rdbFilename, onLog, stat failure, CONFIG-GET failure, non-Error rejection). Total: 64 tests passing.
- **postgres**: Added `tests/unit/lib/adapters/database/postgres/dialects.test.ts` with 33 tests achieving 100% coverage for all PostgreSQL dialect classes - `getDialect()` version detection (PG 12 to 17, PG 15 compatibility mapping, unknown fallback, no-version fallback), `PostgresBaseDialect` (`supportsVersion`, `getDumpArgs` with single-DB, multi-DB fallback, empty-database path, plain/double-quoted/single-quoted options, arg count, `getRestoreArgs` empty-array contract, `getConnectionArgs`), and all three versioned dialects (`Postgres14Dialect`, `Postgres16Dialect`, `Postgres17Dialect`) verifying their specific extra flags.
- **mssql**: Added unit test files for the MSSQL adapter achieving 99.4% statement coverage - `analyze.test.ts` (100%), `connection.test.ts` (100% stmt, all connection modes and error paths), `dialects.test.ts` (100%, all query builders), and extended `dump.test.ts`, `restore.test.ts`, `ssh-transfer.test.ts` with TAR archive detection and extraction, extractor error handling, writeStream error propagation, SSH auth variants, Zip Slip guard annotation, and prepareRestore edge cases. Total: 159 tests passing across all MSSQL test files.
- **mongodb**: Added 6 unit test files for the MongoDB adapter achieving high coverage - `analyze.test.ts` (100%), `connection.test.ts` (100% stmt), `dump.test.ts`, `restore.test.ts`, `dialects.test.ts` (100%), and `index.test.ts` (100% on the adapter export). Tests cover TAR manifest analysis, direct and SSH connectivity (test/getDatabases/getDatabasesWithStats), mongodump spawning in direct and SSH modes, mongorestore with single-DB and multi-DB TAR archives, stderr progress logging, readStream error handling, silent close() failures in finally blocks, and all dialect argument builders. Total: 79 tests passing across all 6 MongoDB test files.
- **storage**: Significantly increased storage adapter test coverage - added tests covering `dropboxFetch` internal wrapper (both branches), FTP `resolvePath` without `pathPrefix`, Google Drive folder creation in `resolveOrCreatePath` and download progress tracker transform body, local adapter security error `onLog` callback, zero-size file progress branch, `entry.path` fallback, and `Access denied` rethrow in `list`/`delete`. OneDrive authProvider callback, failed fetch response, and download tracker transform body. Rsync `_sshpassAvailable = false` path via module isolation and non-permission-denied `mkdir` error rethrow. S3 download progress tracker transform body and all delegated methods for `S3AWSAdapter` and `S3R2Adapter`. SFTP upload step callback progress (both `totalSize > 0` and `totalSize = 0` branches). SMB outer `list()` catch when `SambaClient` constructor throws. WebDAV download tracker transform body. Removed dead code `_ensureFolderExists` from `dropbox.ts`. Total: 302 tests passing across all 11 storage adapter test files.
...
- **mysql-dialects**: Added `tests/unit/lib/adapters/database/mysql/dialects.test.ts` with 38 tests achieving 100% coverage across all 5 MySQL/MariaDB dialect files - `getDialect` factory (all 7 routing branches), `MySQLBaseDialect` dump/restore/connection arg generation, SSL flag handling, multi-database TAR path, custom options splitting, `MySQL57Dialect` and `MySQL80Dialect` supportsVersion + getDumpArgs, `MariaDBDialect` supportsVersion + getDumpArgs.
- **config-runner**: Achieved 100% statement, branch, and line coverage on `src/lib/runner/config-runner.ts` - added 11 new tests covering adapter-not-registered (ConfigurationError, line 56), resolveAdapterConfig parse error swallowed in catch (line 64), encryption profile paths (profile found + decrypt success covering lines 74-85 / 121-125 / 140, decrypt failure throwing EncryptionError, profile missing with and without includeSecrets), safeToIncludeSecrets true-branch (includeSecrets=true + working profile, line 99), temp file cleanup failure logged as warn (line 204), adapter.list error in retention swallowed by outer catch (line 241), adapter.delete error per-file caught individually (line 233), and retention disabled via count=0 (line 208 false-branch). Changed `createEncryptionStream` mock from `mockReturnValue` to `mockImplementation` to prevent stream reuse across tests.
- **runner**: Added `tests/unit/runner/steps/02-dump.test.ts` with 14 tests for the core dump step - single/multi-DB metadata labeling, database auto-discovery via `getDatabases`, pgCompression injection, adapter-type injection (dialect selection), Multi-DB TAR detection, dump failure error propagation, and context guard assertions.
- **integrity**: Added `tests/unit/services/integrity-service.test.ts` with 9 tests covering `IntegrityService.runFullIntegrityCheck` - empty destination set, files without checksums (skip), pass/fail checksum results, fallback to job names when root listing fails, unknown adapter handling, multi-destination accumulation, per-destination error isolation, and the download-based metadata fallback path.
- **auth**: Brought `src/lib/auth/access-control.ts` and `src/lib/auth/download-tokens.ts` to 100% coverage - added 21 tests across `access-control.test.ts` and a new `download-tokens-cleanup.test.ts`. New tests cover `getCurrentUserWithGroup` (session error, no user, user with/without group, first-user SuperAdmin auto-promotion), `checkPermission` (AuthenticationError, no-group PermissionError, SuperAdmin bypass, present/missing permissions, invalid JSON), `getUserPermissions` (all branches including SuperAdmin and malformed JSON), `hasPermission` (true/false paths), the `getAuthContext` fallback when session user has no group, and the download token cleanup interval triggered via fake timers including module hot-reload guard branches. Excluded `src/lib/auth/index.ts` and `src/lib/auth/client.ts` from coverage - both are pure framework configuration files (`betterAuth` init, `createAuthClient`) with no testable business logic.
- **lib**: Brought `runner.ts`, `utils.ts`, `temp-dir.ts`, and `url-validation.ts` to 100% coverage. Added `tests/unit/lib/utils.test.ts` (22 tests: `cn`, `formatBytes`, `formatDuration`, `formatTwoFactorCode`, `compareVersions` including MariaDB/PostgreSQL version string parsing and the empty-version edge case), `tests/unit/lib/temp-dir.test.ts` (4 tests: `getTempDir` with/without TMPDIR env, `getTempPath`), `tests/unit/lib/url-validation.test.ts` (12 tests: valid http/https, localhost, private IPs, invalid URL format, disallowed schemes, all three blocked cloud metadata hosts). Extended `runner.test.ts` with 5 new tests: legacy string log normalization, flush error swallowing, `ctx.updateProgress`/`ctx.updateDetail`/`ctx.updateStageProgress` coverage via mock step, `checkCancelled` throw via abort-without-throw pattern, and `processQueue` rejection swallowed silently by the fire-and-forget `.catch` handler. Added `/* v8 ignore start/end */` around the early dead-code ctx stub in `performExecution`. Excluded `src/lib/prisma.ts` (PrismaClient singleton with `$extends` decryption hook - framework init, not unit-testable).
- **execution**: Added `tests/unit/lib/execution-abort.test.ts` (11 tests) covering the abort registry - register, unregister, abort signal propagation, unknown-ID handling, and concurrent execution tracking. Added `tests/unit/lib/execution-recovery.test.ts` (10 tests) covering `recoverStaleExecutions` - no stale executions, Running/Pending recovery, log append, endedAt update, malformed logs, multi-execution, per-execution error isolation, and `findMany` failure.
- **oidc**: Added unit tests for all four OIDC adapters (`authentik`, `pocket-id`, `keycloak`, `generic`) - 61 tests total, covering adapter metadata, input definitions, Zod schema validation, `getEndpoints` URL construction (including trailing slash stripping), response mapping, non-ok status errors, network failures, SSRF/metadata endpoint blocking, and synchronous vs. async behaviour.
- **restore**: Added `tests/unit/services/restore-smart-recovery.test.ts` with 4 tests for `resolveDecryptionKey` - direct profile hit, missing profile with no candidates (throws), smart-recovery warning log, and profile iteration exhaustion.
- **storage**: Added unit tests for 5 storage adapters - `tests/unit/adapters/storage/s3.test.ts` (25 tests) covering all 4 S3 variants (Generic, AWS, R2, Hetzner) - upload/download/list/delete/read/test operations, pathPrefix wiring, `httpUploadProgress` event capture, and variant endpoint configuration. `tests/unit/adapters/storage/sftp.test.ts` (18 tests) for SFTP upload/download/list/delete/test including directory creation, `fastGet` progress, and recursive walk. `tests/unit/adapters/storage/webdav.test.ts` (18 tests) for WebDAV upload/download/read/list/delete/test with pipeline and progress tracking. `tests/unit/adapters/storage/ftp.test.ts` (19 tests) for FTP upload/download/list/delete/read/test including the `cd("/")` CWD-reset after `ensureDir`. `tests/unit/adapters/storage/smb.test.ts` (19 tests) for SMB upload/download/read/list/delete/test via `samba-client`.
- **notifications**: Expanded notification adapter unit coverage with focused fallback tests for Discord, Email, Generic Webhook, Gotify, ntfy, Slack, Teams, Telegram, and Twilio SMS. Added realistic cases for default values, header and payload fallbacks, status text error handling, and adapter-specific formatting branches to bring notification adapter line coverage to 100 percent.
- **adapters/common**: Brought `src/lib/adapters/database/common/dialect.ts` and `src/lib/adapters/database/common/tar-utils.ts` to 100% coverage. Added `tests/unit/adapters/database/common/dialect.test.ts` with a concrete `BaseDialect` subclass verifying the default `supportsVersion` return value. Extended `tar-utils.test.ts` with two new error-path tests: `extractMultiDbTar` rejecting with "Failed to parse manifest" when `manifest.json` contains invalid JSON, and `readTarManifest` returning `null` for the same case. Marked unreachable infrastructure error-handlers (stream/extractor `.on("error")` callbacks, Zip Slip dead code, `isMultiDbTar` outer catch) with `/* v8 ignore start/end */`. Excluded `src/lib/adapters/database/common/types.ts` from coverage - purely TypeScript interface declarations with no executable code.
- **rate-limit**: Added `tests/unit/lib/rate-limit/index.test.ts` (10 tests) covering `RATE_LIMIT_DEFAULTS` values, `RATE_LIMIT_KEYS` string constants, `getAuthLimiter`/`getApiLimiter`/`getMutationLimiter` return instances, and `applyExternalConfig` rebuilding all three limiters with new points and duration values. Added `tests/unit/lib/rate-limit/server.test.ts` (8 tests) covering `reloadRateLimits` (DB values applied via `applyExternalConfig`, missing/NaN/zero settings fall back to defaults, Prisma error swallowed silently) and `getRateLimitConfig` (parsed values, default fallback, Prisma error returns defaults).
- **runner**: Added `tests/unit/runner/config-runner.test.ts` (7 tests) covering the `runConfigBackup` function - early exit when the feature flag is disabled or absent, early exit when no storage destination is configured, `ConfigurationError` throw when the storage adapter config record is missing, `ConfigurationError` throw when secrets are requested without an encryption profile, retention policy deleting the oldest backup beyond the keep count, and retention policy leaving files untouched when within the limit.
- **mysql**: Added 5 unit test files for the MySQL/MariaDB adapter covering all non-SSH code paths. `tools.test.ts` (7 tests, 100% coverage) - command detection with mariadb/mysql fallback, module-level cache, pre-init fallback values. `analyze.test.ts` (10 tests, 100% coverage) - TAR manifest extraction, USE/CREATE DATABASE/Current Database grep parsing, deduplication, grep no-match (exit code 1), and unexpected grep errors. `connection.test.ts` (16 tests) - `test()` success/failure/disableSsl, `getDatabases()` filtering and --skip-ssl branch, `getDatabasesWithStats()` tab-separated parsing and disableSsl branch, `ensureDatabase()` with and without privileged GRANT mode. `dump.test.ts` (11 tests) - single-DB dump, mysqldump non-zero exit, empty dump file, stderr filter (Using a password/Deprecated program name), database auto-discovery, no-databases-found error, comma-separated and array database lists with multi-DB TAR archive creation and temp directory cleanup on failure. `restore.test.ts` (19 tests) - `prepareRestore()` with and without privilegedAuth, single-DB restore via databaseMapping, `createStderrHandler` behavior (forwarding, filtering, ERROR-level routing, truncation at 500 chars, suppression after 50 lines, flush of remaining buffer), multi-DB TAR restore with selective skipping and cleanup on failure, missing-file error. Total: 63 tests passing, raising mysql coverage from under 3% to `analyze.ts` 100%, `tools.ts` 100%, `connection.ts` 44%, `dump.ts` 63%, `restore.ts` 58% (remaining uncovered lines are exclusively SSH-specific paths covered by integration tests).
