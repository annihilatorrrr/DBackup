...
- **credentials**: Added Phase 3 of the Generic Credential Profile System - a dedicated Credentials manager UI inside the Security Vault. The Vault page now shows a tabbed layout (Encryption / Credentials), and the new `CredentialProfilesList` provides full lifecycle management: create new profiles via a type-aware dialog (USERNAME_PASSWORD, SSH_KEY with conditional `authType` fields, ACCESS_KEY, TOKEN, SMTP), edit metadata, rotate the secret payload, view live reference counts, delete only when no adapter still references the profile, and (for users with `CREDENTIALS.REVEAL`) reveal the decrypted payload with one-click copy
- **credentials**: Added Phase 4 of the Generic Credential Profile System - adapter form integration. The adapter create/edit dialog now exposes a `<CredentialPicker>` for the primary slot (and the SSH slot for adapters with a remote tunnel), automatically filtered by the adapter's declared `CredentialType`. Inline secret fields (`user`/`username`/`password`, `accessKeyId`/`secretAccessKey`, `appToken`/`accessToken`/`botToken`, `sshUsername`/`sshPassword`/`sshPrivateKey`/`sshPassphrase`, etc.) are now hidden whenever the adapter consumes a credential profile - users only manage structural config (host, port, options) inline. Profiles can be created inline via the picker without leaving the form. The `Test Connection` and `Access Check` endpoints transparently overlay the referenced profile so unsaved configs validate end-to-end. The adapter list page surfaces an upgrade banner when adapters are flagged OFFLINE because they're missing a required credential profile, with a direct link to the Security Vault
- **credentials**: Added Phase 5 of the Generic Credential Profile System - runtime + restore path migration. All runtime adapter operations (backup runner steps `01-initialize`, `02-dump`, `04-completion`, `config-runner`, restore service, healthcheck, dashboard storage stats, integrity check, system task version probe, system notification dispatch, storage service, and the runtime API routes for `databases`, `database-stats`, and `storage/analyze`) now resolve their adapter config through `resolveAdapterConfig()`. Credential profile payloads are merged transparently at every callsite, so backups, restores, health checks, and notifications all honor profile changes in real time without per-adapter code changes
